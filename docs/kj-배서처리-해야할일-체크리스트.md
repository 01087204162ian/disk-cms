...
#### 1.1 입력 파라미터 추가 및 검증
- [x] `endorseProcess` 파라미터 추가 (선택, 기본값: "청약")
- [x] `manager` 파라미터 추가 (필수)
- [x] `reasion` 파라미터 추가 (선택)
- [x] 입력 값 검증 로직 추가

#### 1.2 기존 데이터 조회
- [x] `2012DaeriMember` 테이블에서 현재 데이터 조회
  - `push`, `sangtae`, `CertiTableNum` 등 필수 필드 조회
- [x] 이미 처리된 건 검증 (sangtae=2면 에러)
- [x] `2012CertiTable` 테이블에서 `divi` 값 조회 (SMS 발송 조건 확인용)

#### 1.3 push/cancel 값 계산 로직 구현
- [x] 현재 `push` 값 확인 (1 또는 4)
- [x] `endorseProcess` 값에 따른 `push`, `cancel` 값 계산
  - push=1 (청약 상태)일 때:
...
#### 1.4 데이터베이스 업데이트
- [x] `sangtae`, `push`, `cancel`, `manager`, `reasion` 필드 업데이트
- [x] 트랜잭션 처리 (필요 시)

#### 1.5 응답 형식 개선
- [x] 성공 응답에 업데이트된 데이터 포함 (push, cancel, manager 등)
- [x] 에러 응답 개선

### 2. Node.js 프록시 라우터 확인/수정 (`kj-driver-company.js`)

- [x] `/kj-endorse/update-status` 라우트 존재 확인
- [x] POST 요청 처리 확인
- [x] PHP API로 요청 전달 확인

### 3. 프론트엔드 확인 (`kj-driver-endorse-list.js`)

- [x] `updateEndorseStatus` 함수 확인
- [x] API 호출 시 필요한 파라미터 전달 확인:
  - `num` ✅
  - `sangtae` ✅
  - `endorseProcess` (선택한 값)
  - `manager` (로그인 사용자 이름)
  - `reasion` (선택 사항)
- [x] 응답 처리 및 UI 업데이트 확인
...
- [x] `calculateProRatedFee` 함수 호출 준비
- [x] 필요한 데이터 조회:
  - `kj_premium_data` 테이블 (월보험료)
  - `kj_insurance_premium_data` 테이블 (10회 분납 보험료)
  - 나이 계산 (`calculateAge` 함수)
  - 할인할증률 조회 (`2019rate` 테이블)
- [x] 보험료 계산 실행 (청약/해지 처리 시)
- [x] 계산 결과 저장 (필요 시)

### 2. SMS 발송

- [x] SMS 발송 조건 확인 (`sendSms` 플래그)
- [x] `kj-sms-utils.php`의 `sendEndorseSms` 함수 호출
- [x] SMS 메시지 생성 로직:
...
- [x] 청약 처리 시 현대해상(insuranceCompany=4)인지 확인
- [x] `kj-hyundai-api.php`의 `sendHyundaiDriverInfo` 함수 호출
- [x] 암호화 처리 (`encryptForHyundai` 함수)
