...
1. **정산 모달 UI 개선**
   - 인원 컬럼 하단에 합계 인원 표시 추가
   - 보험료, 1/10보험료, 배서보험료, 계, 환산보험료 컬럼 폭 통일 (12%)
   - 10회 분납(divi=1)인 경우 환산보험료 숨김 처리

2. **배서리스트 조회 기능 추가**
   - 배서리스트 전용 모달 생성 (기존 정산 모달 좌측에 표시)
   - 배서리스트 조회 버튼 클릭 시 새 모달 표시
   - 기존 정산 모달과 side-by-side로 동시 표시
   - 모달 위치: 배서리스트 모달(좌측), 정산 모달(우측)

3. **배서보험료 계산 로직 수정**
   - 납부방식에 따라 다르게 계산:
     - 월납(divi=2): `preminum`만 사용
     - 10회분납(divi=1): `c_preminum`만 사용
   - 백엔드와 프론트엔드 모두 수정
...
4. **배서리스트 표시 로직 개선**
   ...
   - 배서종류 표시 (해지/청약)
   - 부호 표시: push=2(해지)는 음수(-), push=4(청약)는 양수(+)
   - push 값 타입 문제 해결 (문자열 → 숫자 변환)

5. **PHP 파일 네이밍 규칙 적용**
   - `kj-settlement-monthly-endorse-search.php` 업데이트
   - 기존 코드를 네이밍 규칙에 맞게 정리

#### 📁 생성/수정된 파일

**수정된 파일**:
- `pci0327/api/insurance/kj-settlement-adjustment.php` - 배서보험료 계산 로직 수정 (납부방식별)
- `pci0327/api/insurance/kj-settlement-monthly-endorse-search.php` - 배서리스트 조회 API 업데이트
- `disk-cms/public/pages/insurance/kj-driver-company.html` - 배서리스트 모달 추가, 테이블 헤더 스타일 수정
- `disk-cms/public/js/insurance/kj-driver-company.js` - 배서리스트 조회 기능, 보험료 표시 로직, 부호 처리 수정
- `disk-cms/routes/insurance/kj-driver-company.js` - 배서리스트 조회 API 프록시 추가

#### 🔧 기술 세부사항

**배서보험료 계산 방식**:
- 월납(divi=2):
  - push=2 (해지): `preminum` (음수)
  - push=4 (청약): `preminum` (양수)
- 10회분납(divi=1):
  - push=2 (해지): `c_preminum` (음수)
  - push=4 (청약): `c_preminum` (양수)

**배서리스트 모달 구조**:
- 좌측에 배서리스트 모달 (48% 폭)
- 우측에 정산 모달 (48% 폭)
- 두 모달 동시 표시 가능 (backdrop: false)
- z-index: 배서리스트 모달(1056), 정산 모달(1055)

**배서리스트 테이블 컬럼**:
- 번호, 성명, 주민번호(마스킹), 증권번호, 배서일
- 월보험료 (월납인 경우만 표시)
- 1/10 보험료 (10회분납인 경우만 표시)
- 배서종류 (해지/청약)
- 처리 (미정산/정산완료 드롭다운)
- 처리자

**주요 버그 수정**:
- push 값 타입 문제: 문자열 비교 → 숫자 변환 후 비교
- 배서종류 표시: push 값 타입 문제로 인한 미표시 해결
- 부호 표시: push=2일 때 음수(-)로 올바르게 표시
